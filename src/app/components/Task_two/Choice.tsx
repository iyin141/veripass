...
const Choice = () => {
  return (
    <div className={`${font.className} pl-[7%] pr-[5%] pb-[5%] flex flex-col gap-[5rem]`}>
      <div className='flex justify-between gap-12 max-sm:flex-col max-sm:text-center max-sm:gap-6'>
        <h1 className='font-semibold text-[3.5vw] max-sm:text-[5vw] w-[50%] max-sm:w-[100%]'>Why Choose VeriPass?</h1>
        <p className='w-[50%] max-sm:w-[100%] '>While many visa service providers only handle basic documentation, VeriPass offers a full-service, done-for-you experience. Here’s what sets us apart:</p>
      </div>
      <div className='flex justify-between max-sm:flex-col max-sm:gap-5 max-sm:justify-center'>
        {info.map((i) => (
          <div key={i.header} className='w-[30%] max-sm:w-[90%] mx-auto flex flex-col gap-5 justify-center items-center text-center bg-[#F2F2F2] md:h-[23rem]  max-sm:p-12 pt-5 pb-5 pr-6 pl-6 rounded-[40px]'>
